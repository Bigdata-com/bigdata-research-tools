...
def fuzzy_find_watchlist_by_name(
    name: str, bigdata: Bigdata, only_private: bool = False, max_distance: int = 2
) -> Watchlist | None:
...
def find_watchlist_by_name(
    name: str, bigdata: Bigdata, only_private: bool = False
) -> Watchlist | None:
    watchlists = bigdata.watchlists.list(owned=only_private)
    for wl in watchlists:
...
def create_watchlist(
    name: str, company_names: list[str], bigdata: Bigdata
) -> Watchlist:
    ...
    for company_name in company_names:
        ...
        entity_list.append(
            bigdata.knowledge_graph.find_companies(company_name, limit=1)[0]
        )
