...
class PortfolioConstructor:
    ...
    def construct_portfolio(
        ...
        portfolio = self._enforce_constraints(
            portfolio, max_position_weight, max_category_weight, balance_col
        )
    ...
    def _balance_by_category(
        self, df: pd.DataFrame, score_col: str, balance_col: str, size: int
    ) -> pd.DataFrame:
        """
        ...
        df = df.copy()
        df["rank"] = df.groupby(balance_col)[score_col].rank(
            ascending=False, method="first"
        )
        mask = df.apply(
            lambda x: x["rank"] <= allocations.get(x[balance_col], 0), axis=1
        )
    ...
    def _enforce_constraints(
        self, df: pd.DataFrame, max_pos: float, max_cat: float, balance_col: str
    ) -> pd.DataFrame:
        """
        ...
        if max_pos <= 0 or max_pos > 1:
            raise ValueError(
                f"max_position_weight must be between 0 and 1, got {max_pos}"
            )
        if max_cat <= 0 or max_cat > 1:
            raise ValueError(
                f"max_category_weight must be between 0 and 1, got {max_cat}"
            )
        ...
        if max_pos * n_positions < 1.0:
            warnings.warn(
                f"Position constraint may be too tight: {max_pos:.1%} * {n_positions} positions = {max_pos * n_positions:.1%} total"
            )

        if max_cat * n_categories < 1.0:
            warnings.warn(
                f"Category constraint may be too tight: {max_cat:.1%} * {n_categories} categories = {max_cat * n_categories:.1%} total"
            )
        ...
        while iteration < self.max_iterations:
            ...
            if prev_weights is not None:
                ...
                if weight_change < self.tolerance:
                    warnings.warn(
                        f"Constraint enforcement converged with tolerance {self.tolerance:.2e} after {iteration} iterations"
                    )
                    break
        ...
        if iteration >= self.max_iterations:
            warnings.warn(
                f"Constraint enforcement stopped after {self.max_iterations} iterations without full convergence"
            )
