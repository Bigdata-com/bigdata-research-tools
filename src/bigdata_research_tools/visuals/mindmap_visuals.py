...
class MindmapPlotter:
    ...
    def __init__(
        ...
        if 'Main Branches' in self.df.columns and 'Sub-Branches' in self.df.columns:
            ...
            self.df = self.df.rename(columns={'Parent': 'Main Branches', 'Label': 'Sub-Branches'})
        else:
        ...
        assert 'Main Branches' in self.df.columns, "Missing 'Main Branches' column after processing"
        assert 'Sub-Branches' in self.df.columns, "Missing 'Sub-Branches' column after processing"
        ...
        self.linewidth_padding = (self.max_linewidth / 72.0) * 3.0  # Very conservative padding
        self.sub_linewidth_padding = (self.sub_linewidth / 72.0) * 3.0  # Sub-branch linewidth padding
        # Use minimal top padding, extra bottom padding
        # Reduce available_height to account for linewidth extension at bottom
        self.available_height = self.fig_height - self.title_height - self.top_padding - (self.bottom_padding + self.sub_linewidth_padding)
        self.available_width = self.fig_width - 2 * (self.edge_padding + self.linewidth_padding)
        ...
        self.absolute_bottom = self.fig_height - self.bottom_padding - self.sub_linewidth_padding
    ...
    def _fit_text_main(
        ...
        available_h = max(0.1, max_height - self.pad_y)
        ...
        max_attempts = 60
        attempt = 0
        while (w > max_width * 0.88 or h > max_height * 0.88) and fontsize > min_font * 0.7 and attempt < max_attempts:
            attempt += 1
            scale = min(max_width * 0.88 / max(0.01, w), max_height * 0.88 / max(0.01, h)) * 0.97
        ...
        if w > max_width * 0.88 or h > max_height * 0.88:
            ...
            scale = min(max_width * 0.88 / max(0.01, w), max_height * 0.88 / max(0.01, h)) * 0.98
            fontsize = max(min_font * 0.7, fontsize * scale)
            chars_per_inch = fontsize / 12.0 * 7
    ...
    def _fit_text_sub(
        ...
        for _ in range(40):
            ...
            wrap_chars = max(25, int(available_w * chars_per_inch))  # Minimum 25 chars, prefer wide lines
        ...
        max_attempts = 30
        attempt = 0
        while (w > max_width * 0.90 or h > max_height * 0.90) and fontsize > min_font * 0.85 and attempt < max_attempts:
            attempt += 1
            scale = min(max_width * 0.90 / max(0.01, w), max_height * 0.90 / max(0.01, h)) * 0.98
        ...
        if w > max_width * 0.90 or h > max_height * 0.90:
            # Force one more reduction
            scale = min(max_width * 0.90 / max(0.01, w), max_height * 0.90 / max(0.01, h)) * 0.99
            fontsize = max(min_font * 0.85, fontsize * scale)
            chars_per_inch = fontsize / 12.0 * 8
    ...
    def _calculate_layout(self) -> Dict:
        ...
        area_per_element = (self.available_height * self.available_width) / max(1, total_elements)
        ...
        main_fonts = []
        for main in self.main_branches:
            font, _, _, _ = self._fit_text_main(ax_temp, main, uniform_main_w, uniform_main_h, base_font * 0.9, 10)
        ...
        main_data = {}
        for main in self.main_branches:
            font, wrapped, w, h = self._fit_text_main(ax_temp, main, uniform_main_w, uniform_main_h, uniform_main_font, 10)
            main_data[main] = {
                'fontsize': font,
                'text': wrapped,
        ...
        sub_fonts = []
        for sub in all_subs:
            font, _, _, _ = self._fit_text_sub(ax_temp, sub, uniform_sub_w * 0.94, uniform_sub_h * 0.94, base_font * 1.0, 9)
        ...
        for main in self.main_branches:
            ...
            for sub in subs:
                font, wrapped, w, h = self._fit_text_sub(
                    ax_temp, sub, uniform_sub_w * 0.94, uniform_sub_h * 0.94, uniform_sub_font, 9
                )
        ...
        return {
            'theme': {'fontsize': theme_font, 'text': theme_wrapped, 'width': theme_w, 'height': theme_h},
            'main_data': main_data,
            'sub_data': sub_data,
    ...
    def _check_overflow(self, ax, layout, x_theme, x_main, x_sub, main_positions, theme_y) -> Dict:
        ...
        theme_text = ax.text(
            ...
            layout['theme']['text'],
            ha='center', va='center', fontsize=layout['theme']['fontsize'],
            fontweight='bold'
        )
        renderer = ax.figure.canvas.get_renderer()
        ...
        if text_w > layout['theme']['width'] * 0.88 or text_h > layout['theme']['height'] * 0.88:
        ...
        for main in self.main_branches:
            ...
            main_text = ax.text(
                ...
                main_info['text'],
                ha='center', va='center', fontsize=main_info['fontsize'],
                fontweight='bold'
            )
            ...
            text_w = bbox.width / ax.figure.dpi
            text_h = bbox.height / ax.figure.dpi
            if text_w > layout['uniform_main_w'] * 0.88 or text_h > layout['uniform_main_h'] * 0.88:
        ...
        for main in self.main_branches:
            ...
            if subs:
                ...
                total_sub_h = layout['uniform_sub_h'] * len(subs) + (len(subs) - 1) * sub_spacing
                ...
                for i, sub_info in enumerate(subs):
                    ...
                    sub_text = ax.text(
                        ...
                        sub_info['text'],
                        ha='center', va='center', fontsize=sub_info['fontsize'],
                        fontweight='bold'
                    )
                    ...
                    text_w = bbox.width / ax.figure.dpi
                    text_h = bbox.height / ax.figure.dpi
                    if text_w > layout['uniform_sub_w'] * 0.90 or text_h > layout['uniform_sub_h'] * 0.90:
    ...
    def plot(self) -> Tuple[plt.Figure, plt.Axes]:
        ...
        for iteration in range(max_iterations):
            ...
            total_h = layout['uniform_main_h'] * self.n_main + layout['main_spacing'] * (self.n_main - 1)
            ...
            # Check for overflow
            issues = self._check_overflow(ax, layout, x_theme, x_main, x_sub, main_positions, theme_y)
            
            # If no issues, we're done
            if not issues['theme_overflow'] and not issues['main_overflows'] and not issues['sub_overflows'] and not issues['bottom_cutoff']:
                plt.close(fig)
                break
            
            # Adjust layout based on issues
            if issues['theme_overflow'] or issues['main_overflows'] or issues['bottom_cutoff']:
        ...
        total_h = layout['uniform_main_h'] * self.n_main + layout['main_spacing'] * (self.n_main - 1)
        y_start = self.top_padding + self.linewidth_padding + (self.available_height - total_h) / 2
    ...
    def _draw_elements(self, ax, layout, x_theme, x_main, x_sub, main_positions, theme_y):
        """Draw all mindmap elements."""
        # Draw theme
        theme_rect = patches.Rectangle(
            (x_theme, theme_y), layout['theme']['width'], layout['theme']['height'],
            linewidth=2.5, edgecolor=self.colors['edge'], facecolor=self.colors['background'], zorder=2
        )
        ...
        ax.text(
            ...
            layout['theme']['text'],
            ha='center', va='center', fontsize=layout['theme']['fontsize'],
            color=self.colors['text'], fontweight='bold', zorder=3
        )
        ...
        for main in self.main_branches:
            ...
            main_rect = patches.Rectangle(
                (x_main, main_y), layout['uniform_main_w'], layout['uniform_main_h'],
                linewidth=2.5, edgecolor=self.colors['edge'], facecolor=self.colors['background'], zorder=2
            )
            ...
            ax.plot(
                ...
                [theme_y + layout['theme']['height'] / 2, main_y + layout['uniform_main_h'] / 2],
                color=self.colors['link'], linewidth=3, alpha=0.6, zorder=1, solid_capstyle='round'
            )
            ...
            if subs:
                ...
                if total_sub_h > available_sub_h:
                    max_sp = (available_sub_h - layout['uniform_sub_h'] * len(subs)) / max(1, len(subs) - 1)
                ...
                if ideal_sub_y_start >= min_top_y and ideal_sub_y_start + total_sub_h <= max_bottom_y:
                    sub_y_start = ideal_sub_y_start
                else:
                ...
                for sub_info in subs:
                    ...
                    sub_rect = patches.Rectangle(
                        (x_sub, sub_y), layout['uniform_sub_w'], layout['uniform_sub_h'],
                        linewidth=self.sub_linewidth, edgecolor=self.colors['edge'], facecolor=self.colors['background'], zorder=2,
                        clip_on=True
                    )
                    ...
                    ax.plot(
                        ...
                        [main_y + layout['uniform_main_h'] / 2, sub_y + layout['uniform_sub_h'] / 2],
                        color=self.colors['link'], linewidth=2.5, alpha=0.5, zorder=1, solid_capstyle='round'
                    )
    ...
    def _adjust_layout_for_overflow(self, layout, issues, iteration):
        ...
        if issues['theme_overflow']:
            ...
            font, wrapped, _, _ = self._fit_text_main(
                ax_temp, self.main_theme, layout['theme']['width'], layout['theme']['height'],
                new_font, 9
            )
        ...
        if issues['main_overflows']:
            for main in issues['main_overflows']:
                ...
                font, wrapped, _, _ = self._fit_text_main(
                    ax_temp, main, layout['uniform_main_w'], layout['uniform_main_h'],
                    new_font, 8
                )
            ...
            for main in self.main_branches:
                ...
                for i, sub_info in enumerate(subs):
                    ...
                    font, wrapped, _, _ = self._fit_text_sub(
                        ax_temp, sub_text, layout['uniform_sub_w'] * 0.94,
                        layout['uniform_sub_h'] * 0.94, layout['sub_data'][main][i]['fontsize'], 8
                    )
...
def plot_mindmap(
    ...
    plotter = MindmapPlotter(mindmap, main_theme, title, color_scheme, output_dir, aspect_ratio)
    fig, ax = plotter.plot()
    plotter.save(fig)
    return fig, ax
