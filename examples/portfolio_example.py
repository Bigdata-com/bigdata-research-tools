...
def example_1_basic_equal_weighted():
    ...
    constructor = PortfolioConstructor(max_iterations=1000, tolerance=1e-6)
...
def example_2_market_cap_weighted():
    ...
    logger.info("\nCountry Allocation:")
    country_weights = (
        portfolio.groupby("Country")["weight"].sum().sort_values(ascending=False)
    )
...
def example_3_score_weighted():
    ...
    logger.info("\nIndustry Allocation:")
    industry_weights = (
        portfolio.groupby("Industry")["weight"].sum().sort_values(ascending=False)
    )
...
def example_5_comparison():
    ...
    for weight_method, weight_col, description in methods:
        ...
        logger.info(
            f"Weight Range: {portfolio['weight'].min():.1%} - {portfolio['weight'].max():.1%}"
        )
